...
quest systems begin
	state start begin
		...
		function bahis_sistemi_kontrolleri(ad, vid, kazanan)
			...
			if not ac_rakipadi or ac_rakipadi == nil then
				return
			end
		...
		function bahis_sistemi_delqf()
			pc.delqf("bahis_sistemi")
			pc.delqf("bahis_sistemi_targetid")
		end
		...
		when logout begin
			...
			local bahis_sistemi_targetid_al = pc.getqf("bahis_sistemi_targetid")
			local bahis_sistemi_durum = pc.getqf("bahis_sistemi")
			local bahis_sistemi_myname = pc.get_name()
			if bahis_sistemi_targetid_al != 0 and bahis_sistemi_durum != 0 then
				cmdchat("LuaToPython bahis_sistemi#kapat#")
				systems.bahis_sistemi_sil()
				systems.bahis_sistemi_delqf()
				pc.select(bahis_sistemi_targetid_al)
				if pc.get_name() != bahis_sistemi_myname then
					systems.bahis_sistemi_sil()
					cmdchat("LuaToPython bahis_sistemi#kapat#")
					systems.bahis_sistemi_delqf()
				end
		...
		when login begin
			...
			-- Bahis Sistemi --
			if pc.getqf("bahis_sistemi") != 0 then systems.bahis_sistemi_delqf() end
		...
		when button begin
			...
			if string.find(gelen, "bahis_kabul#") then
				...
				if ac_kontrol_rakipisim then
					...
					pc.select(tonumber(vidBUL))
					if tonumber(vidBENIM) == pc.get_vid() then syschat("<System>: Rakip çevrimdýþý veya uzakta.") cmdchat("LuaToPython bahis_sistemi#kapat#") systems.bahis_sistemi_delqf() return end
			...
			if string.find(gelen, "bahis_teklif#") then
				...
				local vidBENIM = pc.get_vid()
				local nameBENIM = pc.get_name()
				local vidBUL = find_pc_by_name(tostring(bol[3]))
				local bahis_durum_my = pc.getqf("bahis_sistemi")
				if bahis_durum_my == 1 then
					syschat("<System>: Þuanda biriyle zaten oynuyorsun.")
					return
				end
				
				if tonumber(bol[2]) == 1 then
					systems.bahis_sistemi(tostring(bol[3]))
					pc.setqf("bahis_sistemi_targetid", tonumber(vidBUL))
				end
				--end
				
				local oyunKONTROL = 0
				syschat(tostring(vidBUL))
				pc.select(tonumber(vidBUL))
				local deneme=0
				syschat("another player chat, name is: "..pc.get_name())
				--syschat("A aparut o erroare, mai incercati2!")
				--syschat(pc.getqf("bahis_sistemi"))
				local bahis_durum = pc.getqf("bahis_sistemi")
				--if bahis_durum == 3 then
				if bahis_durum != 3 then
					pc.select(tonumber(vidBENIM))
					cmdchat("LuaToPython bahis_sistemi#kapat#")
					systems.bahis_sistemi_delqf()
					systems.bahis_sistemi_sil_kapat(tostring(bol[3]))
					return
				end
				pc.setqf("bahis_sistemi_targetid", tonumber(vidBENIM))
				if tonumber(bol[2]) == 0 then syschat("<System>: Rakip bahis teklifi reddetti.") cmdchat("LuaToPython bahis_sistemi#red#") systems.bahis_sistemi_delqf() return else end
				if tonumber(bol[2]) == 1 then
			...
			if string.find(gelen, "bahis_oyna#") then
				...
				local vidBENIM = pc.get_vid()
				local nameBENIM = pc.get_name()
				local vidBUL = find_pc_by_name(tostring(bol[2]))
				local oyunKONTROL = 0
				pc.select(tonumber(vidBUL))
